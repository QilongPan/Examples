...
class OperateTable(object):
    ...
    def insert(self,id,score):
        ...
        result = self.db.select(f"SELECT * from {table_name} where id = '{id}'")
        if result is not None and len(result) > 0:
            self.db.update(f"update {table_name} set score='{score}' where id = '{id}'")
        else:
            self.db.insert(f"INSERT INTO `{table_name}` (`id`, `score`) VALUES ({id}, {score})")       
